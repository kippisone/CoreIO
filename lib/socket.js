...
module.exports = function (CoreIO) {
  ...
  var Socket = function Socket(conf) {
    conf = conf || {};
  ...
  Socket.prototype.getSocketServer = function () {
    ...
    if (!Socket.__socketServerInstances[this.host + ':' + this.port]) {
      ...
      (function () {
        ...
        server = CoreIO.getHttpServer(CoreIO.socketHost, CoreIO.socketPort);


        socketServer.installHandlers(server, { prefix: '/' + _this.path.replace(/^\//, '') });
        ...
        Socket.__socketServerInstances[_this.host + ':' + _this.port] = socketServer;
      })();
    }
  ...
  Socket.prototype.start = function () {
    ...
    return new Promise(function (resolve, reject) {
      _this2.__socket = _this2.getSocketServer();
      _this2.__channel = _this2.getChannel();
      resolve();
    });
  };
  ...
  Socket.prototype.monitor = function () {
    ...
    if (!this.__socket.monitoring) {
      ...
      this.__socket.monitoring.stats = function () {
        return {
          connections: _this3.__socket.connections.length,
          channels: Object.keys(_this3.__socket.channels)
        };
      };
    }
