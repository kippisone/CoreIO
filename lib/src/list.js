...
module.exports = function (CoreIO) {
   ...
   */
  var List = function List(name, conf) {
    var _this = this;
  ...
  List.prototype.push = function (data, options) {
    ...
    if (length) {
      ...
      var _data = models.map(function (model) {
        return model.toJSON();
      });
      if (!options.silent) {
        this.emit('item.push', _data, length);
      }

      if (!options.noSync) {
        if (typeof this.sync === 'function') {
          this.sync('push', _data);
        }
      }
    }
  ...
  List.prototype.fetch = function () {
    ...
    if (this.__service) {
      ...
      return this.__service.find.apply(this.__service, args).then(function (data) {
        _this2.push(data, {
          noAutoSave: true
        });
